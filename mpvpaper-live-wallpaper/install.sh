...
# ── Step 4: Create live wallpaper directory + copy transcode script ──────────
...
# Copy transcode-wallpapers.sh to wallpaper directory for easy access
TRANSCODE_SRC="$SCRIPT_DIR/scripts/transcode-wallpapers.sh"
if [ -f "$TRANSCODE_SRC" ]; then
    cp "$TRANSCODE_SRC" "$WALLPAPER_DIR/transcode-wallpapers.sh"
    chmod +x "$WALLPAPER_DIR/transcode-wallpapers.sh"
    ok "transcode-wallpapers.sh → $WALLPAPER_DIR/"
else
    warn "transcode-wallpapers.sh not found in scripts/ — copy manually"
fi
...
echo "    2. (Recommended) cd ~/Pictures/Wallpapers/live-wallpaper && bash transcode-wallpapers.sh"
echo "    3. Press  Super+Alt+W  to cycle live wallpapers"
